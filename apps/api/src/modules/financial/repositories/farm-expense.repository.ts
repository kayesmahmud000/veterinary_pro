...
import {
  FarmExpenseQueryFilter,
  FarmExpenseSummaryFilter,
  IFarmExpenseRepository,
} from "./farm-expense.repository.interface";
...
@Injectable()
export class FarmExpenseRepository implements IFarmExpenseRepository {
  private readonly logger = new Logger(FarmExpenseRepository.name);
  ...
  public async getSummary(
    ...
    // Fetch all matched transactions to aggregate category breakdowns and daily timeline
    ...
    const timeline: ExpenseTimelineItemDto[] = Array.from(
      timelineMap.entries()
    ).map(([date, val]) => ({
      date,
  ...
  private toDomain(record: FarmTransactionWithRelations): FarmExpenseEntity {
